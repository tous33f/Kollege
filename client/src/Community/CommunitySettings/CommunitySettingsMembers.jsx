    ...
    <div>
        ...
        <ul className="space-y-4">
            ...
            <li key={member.id} className="flex items-center justify-between bg-slate-800 p-4 rounded-md">
                ...
                {member?.avatar_url ? <img src={ ` http://localhost:8080/images/${member?.avatar_url} ` } alt={member?.firstname} className="w-11 h-11 ml-5 rounded-full" />:<div className='ml-5 font-medium text-xl cursor-pointer bg-slate-900  px-4 py-2 rounded-full'>{member.username[0].toUpperCase()}</div>}
