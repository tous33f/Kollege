    ...
    <div>
        ...
        <ul className="space-y-4">
            ...
            <li key={request?.username} className="flex items-center justify-between bg-gray-800 p-4 rounded-md">
                ...
                {request?.avatar_url ? <img src={` http://localhost:8080/images/${request?.avatar_url} `} alt={request?.firstname} className="w-10 h-10 rounded-full ml-5" />:<div className='ml-5 font-medium text-xl cursor-pointer bg-slate-900  px-4 py-2 rounded-full'>{request.username[0].toUpperCase()}</div>}
